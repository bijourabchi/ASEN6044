...
plot_NLS(x0true,x0g,ystacked,t,Rb)
...
%%% Plotting Functions
function [] = plot_NLS(x0t,x0NLS,Y,tvec,R)
    ...
    for k = 1:N
        ...
        H(2*k-1 : 2*k, :) = jcb(x0NLS,80000,tvec(k));
    end
    ...
    nexttile; 
    hold on
    title('Error Time History in \zeta')
    err1 = trajE(1,:)-trajT(1,:);
    plot(tvec,err1,'k',LineWidth=1)
    errorbar(tvec(1),err1(1),2*sqrt(P(1,1)),-2*sqrt(P(1,1)),'horizontal','-o')
    xlabel('Time')
    ylabel('Error')
    ...
    nexttile;
    hold on
    title('Error Time History in a')
    err2 = trajE(2,:)-trajT(2,:);
    plot(tvec,err2,'k',LineWidth=1)
    errorbar(tvec(1),err2(1),sqrt(P(2,2)),sqrt(P(2,2)),'-o')
    xlabel('Time')
    ylabel('Error')
    legend('Error', '2\sigma bounds')
