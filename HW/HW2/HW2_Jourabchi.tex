...
We are given a prior and measurment likelihood given by
\begin{align*}
    P(\theta) = \begin{cases}
...
Since we are given $y$, $P(y)$ is a constant and is not dependant on $\theta$. Thus, our posterior distribution will be 
proportional to the measurment likelihood times the prior.
\[
...
We are given that a continous random variable $\theta$ is uniformly distributed on the interval $[0,1]$, adn that measurments are given by
...
\end{cases}
\]
From the following pdf, we can derive our measurment likelihood $P(y\mid\theta)$. Since our noise is additive, we have $n = y-\theta$. Plugging this into $P(n)$ we have
\[
...
\end{cases}
\]
For a fixed observation y, this is identical to the measurment likelihood, $P(y\mid\theta)$. Applying Bayes Rule, we can derive an expression for our posterior.
\begin{align*}
...
Thus, we know our posterior estimate should be cut off at $\theta = 1$ is $y \geq 1$. However if $y < 1$, we know that theta cannot be larger than $y$. Thus an upperbound on our posterior distribution becomes
...
\]
\begin{enumerate}[label=\alph*)]
    \item $\hat{\theta}_{MMSE} = \mathbb{E}[P(\theta\mid y)]$ by definition. Thus we can evaluate the expected value to recieve an expression for $\hat{\theta}_{MMSE}$.
    \begin{align*}
    ...
    \item
        ...
        \]
        Since this function is monotonic increasing, it's maximum must lie a the end of the interval. Thus, $\boxed{\hat{\theta}_{MAP} = \min(u,1)}$.
\end{enumerate}
\section{}
